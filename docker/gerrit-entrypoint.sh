...
if [ "$1" = "/gerrit-start.sh" ]; then
  ...
  if [ -z "$(ls -A "$GERRIT_SITE/git")" ]; then
    ...
    echo "Remove review_site/git"
    [ ${#DATABASE_TYPE} -gt 0 ] && rm -rf "${GERRIT_SITE}/git" && mkdir "${GERRIT_SITE}/git"
    #Default etc
    echo "Init review_site/etc"
    cp -rf ${GERRIT_HOME}/etc_download/gerrit.config ${GERRIT_SITE}/etc/
  fi

  # Copy libs
  echo "Copy libs ..."
  cp -rf ${GERRIT_HOME}/lib_download/* ${GERRIT_SITE}/lib
  cp -rf ${GERRIT_HOME}/plugins_download/* ${GERRIT_SITE}/plugins
  if [ ! -d "${GERRIT_SITE}/hooks" ]; then
    mkdir ${GERRIT_SITE}/hooks
  fi
  [ -z "${COPY_HOOKS}" ] || cp -rf ${GERRIT_HOME}/hooks_download/* ${GERRIT_SITE}/hooks
  [ -z "${COPY_MAILS}" ] || cp -rf ${GERRIT_HOME}/etc_download/mail/* ${GERRIT_SITE}/etc/mail/
  # Backup gc_log
  ...
  [ -z "${HOOKCOMMANDURL}" ] || set_gerrit_config gerrit.installCommitMsgHookCommand "curl -Lo .git/hooks/commit-msg ${HOOKCOMMANDURL}/tools/hooks/commit-msg && chmod +x .git/hooks/commit-msg"
